...
par(mar=c(8,4,4,8))
graph1 <- barplot(totalplayers,cex.names = .4, cex.axis = 0.4, las = 2)
mtext(side = 1, at = graph1, text = totalplayers, line = -10, cex = 0.4)
```

lab <- as.vector(c(fifadata$League))
text(seq(1, 37, by=1),par("usr")[3] - 0.2, labels = lab, srt = 45, pos = 1, xpd = TRUE)
...
par(mar=c(10,4,4,10))
x <- plot(leaguerating,cex.names = 0.8, cex.axis = 0.5, las = 2, xlab = "")
(mtext(side = 1, text = "League", line = 6))
```

x <- barplot(table(mtcars$cyl), xaxt="n")
labs <- paste(names(table(fifadata$League)), "")
text(cex=1, x=x-.25, y=-1.25, labs, xpd=TRUE, srt=45, pos=2)

p2 + theme(axis.title.x = element_text(face="bold", colour="#990000", size=20),
...
graph4 <- barplot(foottype,cex.names = .8, cex.axis = 0.8, las = 2, 
                  main = "Foot wise distribution", xlab = "Preferred foot", 
                  ylab = "Players count")
...
# Plotting Rating wise distribution of the players 
barplot(totalratings, xaxt ="n", yaxt = "n", 
        main = "Rating wise player distribution",
        xlab = "Ratings(out of 100)", ylab = "Number of players", 
        col = "blue")
par(new = "TRUE")
# Plotting Potential Rating wise distribution of the players
...
newgk <- data.frame(GKHandling = c(88,76,68), GKDiving = c(75,66,58), 
                    GKReflexes = c(82,75,71), GKPositioning = c(84,77,70))
...
# Plotting role of Aggression in Defending attributes and Passing attributes
p <- plot_ly(data = fifadata, x = Defending.attributes, 
        y = Passing.attributes,color = Aggression , mode = "markers") %>% 
  layout(title = "Aggression impact")
p
```
